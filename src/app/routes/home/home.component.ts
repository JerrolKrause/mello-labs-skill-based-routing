...
export class HomeComponent implements OnInit, OnDestroy {
	...
	public ngOnInit() {
		...
		this.api.get('/assets/mock-data/columns-pods.json').subscribe((result: any[]) => this.columns = result);
	...
	public viewChange(view: 'pods' | 'users' | 'products') {
		this.ref.detach();
		this.view = view;
	...
	public typeChange(type: 'products' | 'capabilities') {
		this.ref.detach();
		this.type = type;

		switch (type) {
			case 'products':
				//this.state = { "filters": [], "sorts": [{ "dir": "asc", "prop": "pod" }], "groups": [] };
				this.api.get('/assets/mock-data/columns-products.json').subscribe((result: any[]) => {
					this.columns = [...result];
					this.ref.reattach();
				});
